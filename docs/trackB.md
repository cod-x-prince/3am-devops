...
| M2: FastAPI + WebSocket serving EpisodeFrames    | Hour 16     | 🔄 In Progress | —            |
...
| `train.py` — GymWrapper around IncidentEnv                      | 🔄     | Mock-first bootstrap in place; real env swap pending A |
...
| First training run on `bad_deploy` level 1 starts               | ✅     | Mock scenario bootstrap run verified                   |
...
| Run 100 episodes per scenario (600 total)                           | 🔄     | Eval bootstrap runner created with mock scenario |
| Collect: steps_to_resolution, blast_radius, false_positives, reward | 🔄     | Basic metrics collection implemented             |
| Run programmatic grader on each episode                             | ⬜     |                                                  |
| Run LLM grader on sample of episodes (10 per scenario)              | ⬜     |                                                  |
| Output EvalReport as JSON + pretty table                            | ✅     | JSON-like report output via dataclass            |
| `vs_human_baseline` string: "Agent: Xs \| Human: 4.2hr"             | ✅     | Implemented in eval output                       |
...
| `POST /episode/start` — creates episode, returns episode_id | ✅     | Implemented                                    |
| `WS /episode/stream/{id}` — streams EpisodeFrame at 10fps   | ✅     | Implemented with mock env                      |
| `POST /episode/stop/{id}`                                   | ✅     | Implemented                                    |
| `GET /episode/result/{id}` — final EvalReport               | 🔄     | Implemented basic episode result payload       |
| EpisodeFrame schema matches `API_CONTRACT.md`               | 🔄     | **SYNC POINT with A** pending final field lock |
